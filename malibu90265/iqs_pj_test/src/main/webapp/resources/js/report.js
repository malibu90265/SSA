...
function setViewReport() {
	$.ajax({
		...
		data : {
			...
			report_txt : reportTxt.value
		},
		success : function() {
			var html = '<textarea rows=""';
		},
		error : function(xhr) {
			alert("error html = " + xhr.statusText);
		}
...
function setTeamReport() {
	$.ajax({
		...
		success : function() {
			var html = '<p style="margin-top : 0"><팀 선택></p>';
			
			html += '<table class="hover">';
			html += '<tr>'
			html += '<th style="width : 120px">SI사업팀</th>';
			html += '<th style="width : 120px">커머스사업팀</th>';
			html += '<th style="width : 120px">에너지사업팀</th>';
			html += '<th style="width : 120px">보안사업팀</th>';
			html += '<th style="width : 120px">미디어사업팀</th>';
			html += '<th style="width : 120px">경영지원팀</th>';
			html += '</tr>'
			html += '</table>'
				
			$("#teamReport").html(html);
		},
