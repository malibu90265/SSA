...
//	getNoPlaceEmpList();
	...
	//2015-08-18
	//프로젝트 인력에 대하여 검색을 하였을 때 동명이인이 있는지 확인한다.
	//검색 버튼을 클릭하였을 때 사용자의 리스트를 보여준다.
	$("#historySearch").click(function() {
		$.ajax({				
			type : "POST", 
			url:"/getEmployeeByName.action",
			async : false,
			dataType : "json",
			data:{			
				name	:	$("#inputname").val()
			...
			success : function(data) {
				...
				$.each(data.list, function (index, item) {
					html += "<tr onclick='historysave(\""+item.name+"\")'>";
					html += "<td>" + item.name		+ "</td>";
					html += "<td>" + item.position	+ "</td>";
					html += "<td>" + item.division	+ "</td>";
					html += "</tr>"
				});
				 $(".hoverSearchResultTableThead").html(html);
			},
			error : function(xhr) {
				alert(errorTX);
			}, 
